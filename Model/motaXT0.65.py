...
def main():
    ...
    def get_new_random_edge():
        global net, neurons_per_module, total_neurons, cross_modules, synaptic_reach, meta_network
    ...
    def death_probability(v):
        ...
        global death_method, mean_degree, mean_indegree, outdegrees, indegrees, degrees
    ...
    def pruning_probability(e):
        ...
        global pruning_method, mean_indegree_sq, outdegrees, indegrees
    ...
    def save_network_to_file(suffix="", it=0, where="."):
        ...
        fn = f"{where}/Edges/mota_net_{suffix}{it}.edges"
        net.write_edgelist(fn)

    def save_parameters(args, where="."):
        if not os.path.exists(where):
            os.makedirs(where)
        fn = f"{where}/parameters.txt"
        with open(fn, 'w') as f:
            for a in args:
                f.write(f"{a} = {args[a]}\n")
