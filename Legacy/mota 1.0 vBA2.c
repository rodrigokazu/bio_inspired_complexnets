...
//Definições
//Número de neurônios inicial
#define NNI 20000
//Número de neurônios final
#define NN 150000
//SIN
#define SIN 1
//Número de sinapses
#define NC (NCI+NN-NNI)
#define NCI (SIN*NNI)
...
#define DIFFERENT_SETTINGS 0
//Forçar feed forward?
#define FEED_FORWARD 1
#define FEED_FORWARDI 1
...
int main (int argc, char *argv[]) {
    ...
    for (i=0;i<NCI;i++) {
        if (i % 1000 == 0) printf("%ld / %ld\n",i,NC);
         n1 = FEED_FORWARD ? krand() % (NNI-2) + 1 : krand() % (NNI-1) + 1;
         if (SIN_RANGE == 0) {
           if (PREFER) {
             n2 = FEED_FORWARD ? n1 + choose_weighted(&degs[n1+1],NNI-n1-1) : choose_weighted(degs,NNI);
           } else {
             n2 = FEED_FORWARD ? n1 + 1 + krand() % (NNI-1-n1) : krand() % (NNI-1) + 1;
           }
         } else {
           if (PREFER) {
             n2 = FEED_FORWARD ? n1 + choose_weighted(&degs[n1+1],(SIN_RANGE < NNI-1-n1 ? SIN_RANGE : NNI-1-n1)) : choose_weighted(degs,NNI);
           } else {
             n2 = FEED_FORWARD ? n1 + 1 + krand() % (SIN_RANGE < NNI-1-n1 ? SIN_RANGE : NNI-1-n1) : krand() % (NNI-1) + 1;
           }
         }
         add_edge(n1, n2, i);
     }
    ...
    for (i=NCI;i<NC;i++) {
        ...
        if (DIFFERENT_SETTINGS) {
          n1 = FEED_FORWARDI ? krand() % (mnn-2) + 1 : krand() % (mnn-1) + 1;
         if (SIN_RANGEI == 0) {
           if (PREFERI) {
             n2 = FEED_FORWARDI ? n1 + choose_weighted(&degs[n1+1],mnn-n1-1) : choose_weighted(degs,mnn);
           } else {
             n2 = FEED_FORWARDI ? n1 + 1 + krand() % (mnn-1-n1) : krand() % (mnn-1) + 1;
           }
         } else {
           if (PREFERI) {
             n2 = FEED_FORWARDI ? n1 + choose_weighted(&degs[n1+1],(SIN_RANGEI < mnn-1-n1 ? SIN_RANGEI : mnn-1-n1)) : choose_weighted(degs,mnn);
           } else {
             n2 = FEED_FORWARDI ? n1 + 1 + krand() % (SIN_RANGEI < mnn-1-n1 ? SIN_RANGEI : mnn-1-n1) : krand() % (mnn-1) + 1;
           }
         }                       
        } else {
         n1 = FEED_FORWARD ? krand() % (mnn-2) + 1 : krand() % (mnn-1) + 1;
         if (SIN_RANGE == 0) {
           if (PREFER) {
             n2 = FEED_FORWARD ? n1 + choose_weighted(&degs[n1+1],mnn-n1-1) : choose_weighted(degs,mnn);
           } else {
             n2 = FEED_FORWARD ? n1 + 1 + krand() % (mnn-1-n1) : krand() % (mnn-1) + 1;
           }
         } else {
           if (PREFER) {
             n2 = FEED_FORWARD ? n1 + choose_weighted(&degs[n1+1],(SIN_RANGE < mnn-1-n1 ? SIN_RANGE : mnn-1-n1)) : choose_weighted(degs,mnn);
           } else {
             n2 = FEED_FORWARD ? n1 + 1 + krand() % (SIN_RANGE < mnn-1-n1 ? SIN_RANGE : mnn-1-n1) : krand() % (mnn-1) + 1;
           }
         }
         }
         add_edge(n1, n2, i);
    ...
    for (j=1;j<NN;j++) {
        if (j < 50000 * fi) {
            ...
            if (f != NULL) fclose(f);
            sprintf(fn,"motaBAFinTotalDeg%02d %s%s%s%s%sNNI.NN = %ld.%ld, NCI.NC = %ld.%ld, SR = %ld, RT = %u.txt",fi,ts,PREFER ? " PA " : "",(DIFFERENT_SETTINGS ? PREFERI : PREFER) ? " PAI " : "",FEED_FORWARD ? " FF " : "",(DIFFERENT_SETTINGS ? FEED_FORWARDI : FEED_FORWARD) ? " FFI " : "",NNI,NN,NCI,NC,100*SIN_RANGE/NN,clock()/CLOCKS_PER_SEC);
            fi ++;
            f = fopen(fn,"w");
            if (f == NULL) {
